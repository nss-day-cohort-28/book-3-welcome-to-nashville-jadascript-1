...
function parkFetch(clickValue) {
 return fetch(`https://data.nashville.gov/resource/xbru-cfzi.json?$$app_token=vx0odMXRoly9l5yCnqWKCJykT&${clickValue}`)
  ...
  .then(parksData => parksData.forEach((park) => {
    console.log(park)
    let parkName = elementFactory("p", `${park.park_name}- Address: ${park.mapped_location_address}. Year Established: ${park.year_established} <button onclick="parkFetchItin(event.target.value)">Save</button>`)
